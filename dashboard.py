...
def get_metrics():
    ...
    try:
        ...
        # Processar dados semanais para SLA dos DS
        sla_semanas = [
        ...
        # Processar dados semanais para Cobertura de Carteira
        cobertura_semanas = [
        ...
        # Processar dados semanais para Cancelamento - Churn
        churn_semanas = [
        ...
        # Processar dados semanais para RNR
        rnr_semanas = [
        ...
        metrics_data = [
            {
                ...
                "semanas": csat_semanas,
                "total_mes": {"total": csat_data[12], "cumprido": csat_data[13], "percentual": csat_data[14]},
                "status": "Excelente" if float(csat_data[14]) >= 0.95 else "Atenção",
                "cor": "#00D9FF" if float(csat_data[14]) >= 0.95 else "#FF6B35"
            },
            {
                "metrica": "SLA dos DS",
                "meta_objetivo": "82%",
                "meta_percentual": 0.82,
                "semanas": sla_semanas,
                "total_mes": {"total": sla_data[12], "cumprido": sla_data[13], "percentual": sla_data[14]},
                "status": "Excelente" if float(sla_data[14]) >= 0.82 else "Atenção",
                "cor": "#00D9FF" if float(sla_data[14]) >= 0.82 else "#FF6B35"
            },
            {
                "metrica": "Cobertura de Carteira",
                "meta_objetivo": "100%",
                "meta_percentual": 1.0,
                "semanas": cobertura_semanas,
                "total_mes": {"total": cobertura_data[12], "cumprido": cobertura_data[13], "percentual": cobertura_data[14]},
                "status": "Excelente" if float(cobertura_data[14]) >= 1.0 else "Crítico" if float(cobertura_data[14]) < 0.6 else "Atenção",
                "cor": "#00D9FF" if float(cobertura_data[14]) >= 1.0 else "#8B5CF6" if float(cobertura_data[14]) < 0.6 else "#FF6B35"
            },
            {
                "metrica": "Cancelamento - Churn",
                "meta_objetivo": "1%",
                "meta_percentual": 0.01,
                "semanas": churn_semanas,
                "total_mes": {"total": churn_data[12], "cumprido": churn_data[13], "percentual": churn_data[14]},
                "status": "Excelente" if float(churn_data[14]) <= 0.01 else "Atenção",
                "cor": "#00D9FF" if float(churn_data[14]) <= 0.01 else "#FF6B35"
            },
            {
                "metrica": "RNR",
                "meta_objetivo": "22.5%",
                "meta_percentual": 0.225,
                "semanas": rnr_semanas,
                "total_mes": {"total": rnr_data[12], "cumprido": rnr_data[13], "percentual": rnr_data[14]},
                "status": "Excelente" if float(rnr_data[14]) <= 0.225 else "Atenção",
                "cor": "#00D9FF" if float(rnr_data[14]) <= 0.225 else "#FF6B35"
            }
        ]
...
def get_summary():
    ...
    try:
        ...
        # Usar dados do total do mês (últimos valores)
        ...
        total_metrics = 5  # CSAT, SLA dos DS, Cobertura de Carteira, Churn, RNR
        metrics_above_target = 0
        
        if float(csat_meta_percent) >= 0.95:
            metrics_above_target += 1
        if float(sla_meta_percent) >= 0.82:
            metrics_above_target += 1
        if float(cobertura_meta_percent) >= 1.0:
            metrics_above_target += 1
        if float(churn_meta_percent) <= 0.01: # Alterado para 1%
            metrics_above_target += 1
        if float(rnr_meta_percent) <= 0.225:
            metrics_above_target += 1
        ...
        return jsonify({
            ...
            "data": {
                ...
                "performance_percentage": (metrics_above_target / total_metrics) * 100,
                "team_name": "Lendários"
            }
        })
...
def get_weekly_data():
    ...
    try:
        ...
        weekly_data = {
            ...
            "csat": [
                float(csat_data[2]) * 100,
                float(csat_data[5]) * 100,
                float(csat_data[8]) * 100,
                float(csat_data[11]) * 100 if csat_data[11] != 0 else 0
            ],
            "sla": [
                float(sla_data[2]) * 100,
                float(sla_data[5]) * 100,
                float(sla_data[8]) * 100,
                float(sla_data[11]) * 100 if sla_data[11] != 0 else 0
            ],
            "cobertura": [
                float(cobertura_data[2]) * 100,
                float(cobertura_data[5]) * 100,
                float(cobertura_data[8]) * 100,
                float(cobertura_data[11]) * 100 if cobertura_data[11] != 0 else 0
            ],
            "churn": [
                float(churn_data[2]) * 100,
                float(churn_data[5]) * 100,
                float(churn_data[8]) * 100,
                float(churn_data[11]) * 100 if churn_data[11] != 0 else 0
            ],
            "rnr": [
                float(rnr_data[2]) * 100,
                float(rnr_data[5]) * 100,
                float(rnr_data[8]) * 100,
                float(rnr_data[11]) * 100 if rnr_data[11] != 0 else 0
            ]
        }
